    ...
    <div class="time-capsule">
        ...
        <div v-if="timeData" class="all-capsule">
            ...
            <div v-if="store.siteStartShow" class="capsule-item start">
                ...
                <div class="item-title" style="color: #cf1224;font-weight: bold;">{{ jpnwDateText }}</div>
            </div>
        </div>
    </div>
</template>
...
import { getTimeCapsule, siteDateStatistics, jpnwDateStatistics } from "@/utils/getTime.js";
...
const jpnwDate = ref(import.meta.env.VITE_JPNW_START);
const startDateText = ref(null);
const jpnwDateText = ref(null);
...
onMounted(() => {
  timeInterval.value = setInterval(() => {
      ...
      if (jpnwDate.value) jpnwDateText.value = jpnwDateStatistics(new Date(jpnwDate.value));
  }, 1000);
});
